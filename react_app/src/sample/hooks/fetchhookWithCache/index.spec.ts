...
describe('Fetchモック動作確認', () => {
    ...
    it('サンプルテスト 200-Post', async () => {
        ...
        const { result } = renderHook(() => useFetch<{ message: string }>({ url: './api/sample', method: 'GET' }));
        await waitFor(() => {
            expect(result.current.response).toEqual(data);
            // console.log(result.current)
        });
    ...
    it('サンプルテスト 404', async () => {
        ...
        const { result } = renderHook(() => useFetch<object>({ url: './api/sample/404', method: 'GET' }));
        await waitFor(() => {
    ...
    it('サンプルテスト ネットワークエラー', async () => {
        ...
        const { result } = renderHook(() => useFetch<object>({ url: './api/sample/404' }));
        await waitFor(() => {
